...
export interface IInstallerResult {
  ...
  /** The local path to the installer, or empty if the bundled installer is used. */
...
export interface IEnvProvider {
  label: string;
  /**
   * Whether this provider is requested by the given action inputs.
   */
  provides: (
    inputs: IActionInputs,
    options: IDynamicOptions,
  ...
  /**
   * The args to pass to conda/mamba for environment creation or update.
   */
  condaArgs: (
    inputs: IActionInputs,
    options: IDynamicOptions,
...
/** An asset with extra metadata from the parent release. */
export interface IGithubAssetWithRelease extends IGithubAsset {
  tag_name: string;
}

/** A GitHub release response body from the releases API. */
